import argparse
from data.datasets import OKVQATrainingDataset, PassageDataset
...
from modeling.models import RankerTrainingPipeline, PretrainedConfig, E_MM, E_T, DEDRJointTraining, DEDR
...
def gen_passage_rep(opts, model, collator):
    ...
    for i, batch in enumerate(tqdm.tqdm(dataloader)):
        ...
        elif opts.model_type in ["DEDR_joint", "DEDR"]:
            outputs = model(
...
parser.add_argument("--model_type", required=True, help="model type for training: E_T, E_MM, DEDR_joint, DEDR (only for inference)")
...
parser.add_argument("--model_path", default="", help="path to the model to be evaluated")
...
if __name__ == "__main__":
    ...
    if opts.model_type == "E_MM":
        ...
        model_class = DEDRJointTraining
        config = [BertConfig.from_pretrained("bert-base-uncased"), LxmertConfig.from_pretrained("unc-nlp/lxmert-base-uncased")]
        # config[0].gradient_checkpointing = True

    if not checkpoint_exists and not opts.model_path:
        model = RankerTrainingPipeline(config = config, neg_type = opts.neg_type, model_type = opts.model_type)
        model = model.to(opts.local_rank)
        optimizer, scheduler = optim.set_optim(opts, model)
        step = 0
    elif checkpoint_exists and opts.do_train:
        model, optimizer, scheduler, opt_checkpoint, step = load_checkpoint(model_class, os.path.join(checkpoint_path, "checkpoint", "latest"), opts)
        model = RankerTrainingPipeline(config = config, neg_type = opts.neg_type, model_type = opts.model_type, model = model)
    
    if opts.do_validation and opts.model_path:
        if opts.model_type == "E_MM":
    ...
    if opts.do_validation and opts.is_main:
        evaluate(model, val_dataset, collator, collator_passage, opts, "validation", dynamic_eval, logger)
